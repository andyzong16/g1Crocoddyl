...
# Configuration
OUTPUT_FILE = "stepping_dataset_11.4.npz"
NUM_KNOTS = 30
TIME_STEP = 0.02
...
GRID_X_STEPS = 15  # Number of steps in x direction
GRID_Y_STEPS = 10  # Number of steps in y direction
...
def generate_grid_samples(lfPos0, rfPos0, left_stance_range, right_stance_range, grid_x_steps, grid_y_steps, grid_yaw_steps):
    """
    Generate a grid of foot target positions with separate ranges for each stance.

    Args:
        lfPos0: Initial left foot position
        rfPos0: Initial right foot position
        left_stance_range: Range dict for when left is stance {'x', 'y', 'yaw'}
        right_stance_range: Range dict for when right is stance {'x', 'y', 'yaw'}
    ...
    Returns:
        List of dicts with 'left_target', 'right_target', 'stance_foot', 'swing_foot', 'target_yaw'
    """
    samples = []

    # Generate samples for left foot stance (right foot swings)
    ...
    for dx in x_values_right:
        for dy in y_values_right:
            for yaw in yaw_values_right:
                right_target = rfPos0.copy()
                right_target[0] += dx
                right_target[1] += dy

                samples.append(
                    {"left_target": lfPos0.copy(), "right_target": right_target, "stance_foot": "left", "swing_foot": "right", "target_yaw": yaw}
                )

    # Generate samples for right foot stance (left foot swings)
    ...
    for dx in x_values_left:
        for dy in y_values_left:
            for yaw in yaw_values_left:
                left_target = lfPos0.copy()
                left_target[0] += dx
                left_target[1] += dy

                samples.append(
                    {"left_target": left_target, "right_target": rfPos0.copy(), "stance_foot": "right", "swing_foot": "left", "target_yaw": yaw}
                )
...
def main():
    ...
    grid_samples = generate_grid_samples(lfPos0, rfPos0, LEFT_SWING_RANGE, RIGHT_SWING_RANGE, GRID_X_STEPS, GRID_Y_STEPS, GRID_YAW_STEPS)
    ...
    for i, sample in enumerate(grid_samples):
        ...
        # Compute foot command (displacement in xy)
        if swing_foot == "left":
            foot_cmd_xy = left_target[:2] - lfPos0[:2]
        else:
            foot_cmd_xy = right_target[:2] - rfPos0[:2]

        print(f"Stance foot: {stance_foot}")
        print(f"Foot command: [{foot_cmd_xy[0]:.3f}, {foot_cmd_xy[1]:.3f}], yaw: {target_yaw:.3f} rad")
        print(f"  Left foot: {lfPos0[:2]} -> {left_target[:2]}")
        print(f"  Right foot: {rfPos0[:2]} -> {right_target[:2]}")
        ...
        # Generate waiting frames before stepping
        wait_data_before = generate_waiting_frames(robot, gait, x0, wait_frames_before, left_target, right_target, target_yaw)

        # Solve (without verbose logging)
        solver, success = solve_stepping_problem(gait, x0, left_target, right_target, target_yaw, verbose=False)
